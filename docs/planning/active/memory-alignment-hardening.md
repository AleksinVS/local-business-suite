...
- Основание: дополнение 2026-07-06 в `docs/adr/ADR-0030-memory-alignment-hybrid-knowledge-v05.md` (раздел «Требуемые действия», «Дополнение 2026-07-06») и остаточные риски из `workflow/archive/2026/memory-hybrid-knowledge-v05-alignment/RETROSPECTIVE.md`; кросс-ревью находок параллельного kb-проекта (2026-07-06) добавило канон-ориентированный обход, базисы из git и обновление входящих рёбер при supersede.
...
| `03-edge-emission-and-retro-leftovers` | Инструкция эмиссии `relations:` в конфигурации семантического прохода (`knowledge_reflection_worker`), обновление входящих рёбер при `supersede` (хелпер обратного индекса по `MemoryKnowledgeEdge`), чистка `ranking_profiles` до одного `balanced`, фикс фильтра очереди review-UI |
