...
## Статус и дополнение 2026-05-20
...
Дополняющие документы:

- `docs/architecture/MEMORY_COMPLETION_GAP_ANALYSIS.md` - разрывы между текущей реализацией и целевой системой памяти;
- `docs/architecture/MEMORY_CHAT_REFLECTION_AND_SECRET_HANDLES_PLAN.md` - план chat-derived memory, queued `memory.remember`, sleep-time reflection, candidate promotion и secret handles.

Ключевое изменение к раннему плану: secret-like content больше не должен блокировать весь ingestion snapshot как целевое поведение. Целевое поведение - span-level extraction: secret value уходит в `SecretHandleBackend`, в memory остается только `<SECRET_HANDLE:...>` и metadata, а не-secret текст продолжает ingestion. Текущее MVP-поведение с блокировкой всего snapshot остается известным gap до реализации ADR-0005.
...
Для MVP, где агент только создает/находит ссылки на секреты, а значение вводит и читает сам пользователь, предварительно одобрен Vaultwarden как human-vault хранилище.

OpenBao переносится на дальнейшие этапы для service-consumed secrets: service-to-service секреты, API tokens, dynamic credentials, leases и server-side lookup. Код должен зависеть от provider-neutral `SecretHandleBackend`, а не от конкретного vault.
...
| Secrets | MVP: Vaultwarden для human-entered/human-read secrets через ссылки/handles; later: OpenBao для service-consumed secrets | На первом этапе агент не читает value, а OpenBao нужен позже для machine credentials и leases |
...
Примечание 2026-05-20: для final target секреты не должны блокировать весь документ/сообщение. Требуется span-level processing: secret value извлекается в `SecretHandleBackend`, безопасный текст получает `<SECRET_HANDLE:...>`, а остальной контент продолжает ingestion. До реализации ADR-0005 текущая блокировка всего snapshot считается временным MVP-ограничением.
...
- Secret-like span не попадает в индекс; non-secret контекст продолжает processing.
- Если secret backend недоступен, secret span редактируется/уходит в issue queue, но весь документ блокируется только при невозможности безопасно отделить secret.
...
- MVP baseline: Vaultwarden как предварительно одобренное хранилище для human-entered/human-read secrets;
- agent получает только `<SECRET_HANDLE:...>` и ссылку/metadata, не secret value;
- later phase: OpenBao для service-consumed secrets, dynamic credentials, leases и server-side lookup;
- implementation должен зависеть от единого provider-neutral `SecretHandleBackend`;
- пока service-secret backend не внедрен, использовать private deployment `.env` для service secrets и не передавать значения секретов агенту.
...
Детализация реализации и chat-derived secret flow описаны в `docs/architecture/MEMORY_CHAT_REFLECTION_AND_SECRET_HANDLES_PLAN.md`.
