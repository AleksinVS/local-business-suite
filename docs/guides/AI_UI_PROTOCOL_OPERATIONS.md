...
Документ описывает, как включать и проверять общий AI UI слой. CopilotKit остается отдельным драйвером, а самописный UI использует AG-UI-compatible поток через Django same-origin proxy.
