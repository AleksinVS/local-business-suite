# Корпоративный портал ВОБ №3: Handoff For Next Agent
...
#### Слэш-команды ИИ-чата

ИИ-чат поддерживает слэш-команды двух типов:

- **Предопределённые** — объявлены в `apps/ai/commands.py` (аналогично `tool_definitions.py`):
  - `/навыки` (алиасы: `/skills`, `/ski`) — ИИ описывает свои возможности
  - `/команды` (алиасы: `/commands`, `/cmd`) — панель управления командами

- **Пользовательские** — хранятся в модели `SlashCommand` (per-user), содержат шаблон промпта с плейсхолдером `{input}`

Разрешение команд:
- `resolve_command()` проверяет предопределённые команды и алиасы
- `resolve_custom_command()` проверяет пользовательские команды
- `/команды` возвращает JSON `status: "command_list"` (не создаёт сообщение, рендерит панель управления)
- Остальные команды заменяют промпт на шаблон и отправляются ИИ как обычное сообщение

Ключевые файлы:
- `apps/ai/commands.py` — реестр предопределённых команд и функции разрешения
- `apps/ai/models.py` → `SlashCommand` — модель пользовательских команд
- `apps/ai/views.py` → `SlashCommandListView`, `SlashCommandCreateView`, `SlashCommandDeleteView` — CRUD API
- Шаблон `chat_detail.html` — кнопка меню команд, автодополнение, панель `/команды`, приветственное сообщение с `/навыки`
