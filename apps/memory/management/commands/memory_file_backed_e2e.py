...
from apps.memory.chat_memory import process_queued_memory_requests, queue_memory_remember
...
class Command(BaseCommand):
    help = "Run an end-to-end check of queued remember -> knowledge file -> index -> search."
    ...
    def handle(self, *args, **options):
        ...
        processed = process_queued_memory_requests(limit=10)
        if not processed:
            raise CommandError("Queued memory request was not processed.")
        item = MemoryKnowledgeItem.objects.get(memory_id=processed[0]["memory_id"])
        verification = verify_knowledge_item_file(item)
        if not verification["ok"]:
            raise CommandError(f"Knowledge file verification failed: {verification}")
        result = memory_search(actor=user, query=marker, sensitivity="internal", request_id=f"search-{marker}")
        if not result["items"]:
            raise CommandError("Saved knowledge was not found through memory.search.")
        self.stdout.write(
            self.style.SUCCESS(
                "Memory file-backed e2e succeeded: "
                f"request_id={queued['request_id']}, memory_id={item.memory_id}, file={item.knowledge_file_path}"
            )
        )
