...
Блок выравнивания памяти по ADR-0030 исполнен и заархивирован 2026-07-04. Ревью целевой архитектуры 2026-07-06 (дополнение в ADR-0030) и ретроспектива блока оставили семь пробелов: LF-нормализация канона и `.gitattributes` (без них git-autocrlf на Windows ломает content-hash дисциплину), гарантия отсутствия петли `needs-reconcile`, синглтон `memory_reconcile`, инструкция эмиссии `relations:` для семантического прохода (словарь и материализатор есть, производителя рёбер нет), golden set `MemoryEvalCase` для проверяемых критериев возврата отложенных контуров, матрица восстановления в deployment-документации, чистка `ranking_profiles` и фильтра review-UI.

Контекст:
- основание: `docs/adr/ADR-0030-memory-alignment-hybrid-knowledge-v05.md`, раздел «Требуемые действия», «Дополнение 2026-07-06»;
