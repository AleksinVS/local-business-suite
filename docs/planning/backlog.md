...
Оставшееся действие:
- все 12 пакетов исполнены и приняты (01–12, каждый scoped-коммитом; высокорисковые 01/03 — с независимой проверкой субагентом); блок технически завершён, ожидает приёмки владельцем;
- **следствие пакета 03 (media):** `apps.ai.ChatAttachment` (`/media/chat_attachments/...`,
  шаблон `templates/ai/chat_detail.html`) раздавался тем же снятым `static(MEDIA_URL)` и
  теперь не обслуживается нигде. В prod (`DEBUG=0`) он и раньше давал 404 — новой
  прод-регрессии нет; в dev вложения чата больше не отображаются (раньше — без проверки
  прав, та же уязвимость). Нужен защищённый view для `ChatAttachment` (по образцу
  `serve_workorder_attachment`) либо единый media-dispatcher по префиксу с политикой
  per-domain. Вне scope пакета 03 (`apps/ai/`); кандидат в отдельный пакет 13 — требует
  решения владельца по приоритету;
- **следствие пакета 05 (аудит gateway) — решение владельца:** на синхронном `/chat`
  Django-клиент ждёт `LOCAL_BUSINESS_AGENT_RUNTIME_TIMEOUT=90s`, а agent-runtime доводит
  LLM-вызов до `LLM_DEADLINE_SECONDS=300s` (`services/agent_runtime/graph.py`). Django
  обрубает первым — пользователь получает таймаут, пока рантайм работает; для
  write-инструментов побочный эффект может записаться уже после ответа об ошибке (риск
  дублей). Варианты: (1) поднять `LOCAL_BUSINESS_AGENT_RUNTIME_TIMEOUT` > 330 (но воркер
  gunicorn блокируется на всё это время — capacity-компромисс при 3 prefork-воркерах);
  (2) снизить `LLM_DEADLINE_SECONDS` под sync-таймаут; (3) вести весь чат через стриминг
  (стримовые пути `read=600s` уже согласованы). Значения не менялись — компромисс за
  владельцем; детали в `TASK_ACCEPTANCE.05-*`;
...
Оставшееся действие:
- выполнить полный e2e для `copilotkit` и `native` после перезапуска сервисов с нужными env (драйвер `legacy` выведен, ADR-0032);
