...
- `memory_queue_worker` (единая очередь памяти: reconcile/ingestion/reindex, таблица `MemoryExternalConnectorJob`, ADR-0030 решение 2);
...
`memory.remember` больше не ставит запись в очередь: файл знания, git commit и
индекс создаются синхронно за один вызов (ADR-0030 решение 2). Если инлайн-
индексация не удалась, запись знания все равно сохраняется, а переиндексация
уходит в `memory_queue_worker` с retry и dead-letter. Отдельные таблицы
`MemoryWriteRequest`/`MemoryIndexJob`/`MemoryKnowledgeEvent`/`MemoryReflectionRun`
удалены; `knowledge_writer_worker` и `knowledge_index_worker` больше не нужны.
...
```bash
python manage.py memory_queue_worker --dry-run
