...
Уточнено `docs/adr/ADR-0032-retire-legacy-ai-ui-driver.md` (2026-07-04): драйвер `legacy`,
упоминаемый ниже как часть исходной матрицы `legacy|copilotkit|native`, выведен из проекта.
Допустимая матрица драйверов AI UI - `copilotkit|native`. Текст ниже сохранен как есть для
исторической точности принятого на 2026-06-09 решения.
...
- самописный UI как основной целевой продуктовый ИИ-чат проекта;
- CopilotKit/AG-UI как готовый внешний UI/runtime-слой, эталон совместимости и равноправный клиент общего AG-UI контура.
...
Создать версионируемую основу AI UI протоколов и считать CopilotKit и самописный UI сменными драйверами поверх нее. Основной целевой интерфейс - самописный ИИ-чат, но он не получает отдельный несовместимый backend-контракт: для backend он такой же AG-UI клиент, как CopilotKit.
...
Правила сопровождения версии AG-UI:

- при изменениях backend, которые затрагивают `/ag-ui`, `services.agent_runtime.protocols`, `apps.ai.ui_runtime`, AI tools, Django AI gateway или deployment-профили AI UI, разработчик обязан проверить актуальность зафиксированной версии AG-UI;
- сверять нужно `@ag-ui/client` в `package.json`/`package-lock.json`, `LOCAL_BUSINESS_AI_UI_AGUI_PROFILE` и официальные release notes/docs, если есть риск изменения протокола;
- по умолчанию результат проверки - предупреждение в отчете, а не обновление версии;
- повышение версии `agui_profile`, `@ag-ui/client`, CopilotKit или wire contract выполняется только после явного согласования с владельцем;
- согласованное обновление версии требует обновления ADR/операционных документов и e2e matrix для `legacy`, `copilotkit`, `native`.
