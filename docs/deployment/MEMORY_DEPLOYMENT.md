...
Для памяти из AI-чата проверить совместимую обработку очереди "запомни".
`memory.remember` в обычном AI-инструменте возвращает `status=queued`, `request_id` и `job_id`;
`MemoryKnowledgeItem`, `memory_id` и поисковый индекс появляются после запуска обработчика очереди.
Команда также создает кандидатов в общую память и не является полноценной ночной рефлексией:

```bash
python manage.py memory_reflect_chats --dry-run
```

После запуска без `--dry-run` проверить, что запрос перешел в `accepted`, а `memory.search` находит сохраненный `memory_chunk`.
