...
def _build_memory_section() -> str:
    return "\n".join(
        [
            ...
            "- Если пользователь просит искать точное слово, номер, фразу или термин в исходных файлах, документах, таблицах или по содержимому файла, вызывай `memory.search` с `search_mode=\"source_explicit\"` и `ranking_profile=\"source_content\"`.",
            "- Если пользователь просит искать по смыслу именно в исходных файлах или документах, вызывай `memory.search` с `search_mode=\"source_explicit\"` и `ranking_profile=\"source_semantic\"`.",
            "- Если пользователь просит найти принятое знание по смыслу, похожую инструкцию или говорит, что не помнит точные слова, вызывай `memory.search` с `search_mode=\"knowledge_semantic\"` и `ranking_profile=\"semantic_heavy\"`.",
            "- Если пользователь ищет точное значение в принятой памяти: номер заявки, код, дату, название или цитату, вызывай `memory.search` с `search_mode=\"knowledge_precise\"` и `ranking_profile=\"precise\"`.",
            "- Если достаточно сначала искать принятое знание, но при пустом результате можно показать исходные документы, используй `search_mode=\"source_fallback\"`, `ranking_profile=\"balanced\"` и `include_source_data=true`.",
            "- Не передавай сырые веса каналов поиска; весами управляет серверный `ranking_profile`.",
