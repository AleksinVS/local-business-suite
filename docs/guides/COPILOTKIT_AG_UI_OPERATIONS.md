...
Основной долгосрочный UI проекта - самописный AG-UI-compatible чат. CopilotKit в этом документе рассматривается как отдельный равноправный драйвер, эталон совместимости и production candidate для сравнения, а не как владелец backend-контракта.
...
## Версии AG-UI и CopilotKit

Зависимости AG-UI/CopilotKit закреплены в `package.json` и `package-lock.json`, а серверный профиль - в `LOCAL_BUSINESS_AI_UI_AGUI_PROFILE`.

При изменениях `/ag-ui`, `services.agent_runtime.protocols`, `apps.ai.ui_runtime`, Copilot Runtime, AI tools или deployment-профилей нужно проверить, не вышли ли новые версии AG-UI/CopilotKit с изменением протокола или рекомендуемого runtime-поведения.

По умолчанию проверка не обновляет зависимости. Если найдена новая версия, зафиксировать предупреждение и риск в отчете. Обновлять `@ag-ui/client`, `@copilotkit/*` или `agui_profile` можно только после согласования с владельцем и последующего e2e для `copilotkit` и `native`.
