...
**Статус:** готов к исполнению (старт — решение владельца; ADR создаётся задачей T6 со статусом Proposed) · **Дата:** 2026-07-07
**Проектный документ:** `docs/architecture/project-knowledge-system-v0.1.md` (версия 0.9; далее — «проект»)
**Workflow-блок:** `workflow/active/project-knowledge-base-mvp/` (пакеты 01–06 ↔ задачи T0–T6)
**Целевая аудитория:** агент-исполнитель. Все решения уже приняты — исполнителю запрещено их менять; порядок работы с противоречиями и пробелами — правило 2 ниже.
...
2. Ничего не проектируй заново: DDL, алгоритмы, форматы и коды выхода заданы в этом плане дословно. Если план и проект расходятся — прав этот план (он новее); сообщи о расхождении в отчёте. Пробел (случай, не описанный ни планом, ни проектом) закрывай минимальным решением, согласующимся с проектом, и явно фиксируй его в отчёте; зафиксированные решения (§1) при этом неприкосновенны.
...
| Рабочая директория | kb.py на старте определяет корень репозитория (`git rev-parse --show-toplevel`) и работает от него; все пути в коде — от корня (запуск из поддиректории не ломает поведение) |
...
### 3.6 `docs/knowledge/eval/cases/` (golden set, пофайловый)

Golden set поиска: запрос → ожидаемые страницы (id без `.md`), k=3. Один case = один YAML-файл `eval/cases/<slug>.yaml`; slug — kebab-case темы запроса (латиница/цифры/дефис). Пофайловое хранение выбрано ради параллельных веток: case из разных веток не конфликтуют при слиянии, совпадение имён файлов — сигнал дубля темы. Формат case — ровно эти поля:

```yaml
# eval/cases/khranenie-proekcii.yaml — пример формата
q: "как хранится проекция базы знаний"
expect: ["concepts/dev/knowledge-base"]
```

Пополняется агентом при каждом промахе поиска («страница есть, но не нашлась») и case-файлом «вопрос о решении → её id» для каждой новой страницы `pattern`/`constraint`. Заполняется содержательно в фазе 2; в фазе 0 создать пустой каталог с файлом `.gitkeep` (пример выше в канон не записывать).
...
1. Определить корень репозитория (`git rev-parse --show-toplevel`, §1) — все дальнейшие пути от него. Открыть/создать БД (`.local/knowledge/`, `mkdir -p`); если `--full` — предварительно удалить файл БД.
...
3. Создать схему §4 при отсутствии — **уже под блокировкой** (DDL внутри транзакции SQLite допустим; так два первых sync не гоняются за создание таблиц). Несовпадение `meta.schema_version` → сообщение «выполните kb sync --full», код 2; `CREATE TABLE IF NOT EXISTS` для смены схемы недопустим.
4. Загрузить словари (§5.5).
...
Все сравнения времени — в **секундах epoch**: `%ct` уже epoch; `timestamp` страницы переводить как `datetime.fromisoformat(ts.replace('Z', '+00:00')).timestamp()`. Naive- и aware-datetime напрямую не сравнивать (TypeError).

### 6.3 Материализация

1. Сканировать `docs/knowledge/**/*.md`, исключая `schema/` и файлы с именем `index.md` на **любом** уровне вложенности (корневой индекс и под-индексы доменов — не страницы; используются только lint'ом, §7).
...
3. **Изменённые валидные**: UPSERT в `pages`; `DELETE FROM chunks WHERE page_id=?` и `DELETE FROM chunks_fts WHERE page_id=?`; вставить чанки заново (в обе таблицы, `chunk_id` из `chunks` продублировать в fts-колонку); перед `DELETE FROM edges WHERE src=?` запомнить прежнее множество `(type, dst)` страницы (`SELECT type, dst FROM edges WHERE src=?` в локальную переменную, живёт до конца обработки этой страницы) — оно нужно детектору `contradiction` (§6.4); вставить рёбра, тип которых ∈ `approved_edge_types` (прочие — ошибка E2, не вставляются). После успешной материализации — `DELETE FROM reconcile_queue WHERE page_id=? AND reason='invalid-frontmatter'` (страница починена, причина снимается автоматически).
...
Атрибуция изменённой страницы `p` (путь `f`) — **два независимых источника**, объединяемых по приоритету `human` > `maintenance` > `agent`:

- **коммиты (проверяются всегда, даже если файл сейчас в dirty):** собрать акторов коммитов из батча §6.2, затронувших `f`, новее `meta.last_processed_commit` (при пустом `last_processed_commit` — новее `timestamp` страницы, сравнение в epoch); коммит без трейлера = `human`;
- **незакоммиченное:** если `f ∈ dirty` — добавить актора `human` при `--session-start`, иначе значение `--actor`.

Итоговый актор — максимальный по приоритету из собранных: незакоммиченная правка агента не скрывает более ранний человеческий коммит того же файла.
...
|---|---|---|
| `human-edited` | A | актор изменения = `human` (при `--full` пересчёт: коммиты без трейлера новее `timestamp` страницы). Гаснет только обновлением `timestamp` при обработке — правило замыкания класса A (§7.5 проекта). detail = короткие хэши человеческих коммитов через запятую либо `uncommitted` |
| `source-drift` | A | для каждой валидной страницы: существует путь `s` из `sources`+`covers` (без завершающего `/`; суффикс `@<коммит>`, если есть, отрезать), для которого в `last_touch` есть ключ, равный `s` или начинающийся с `s + '/'`, со временем > `timestamp` страницы (сравнение в epoch, §6.2). Проверяется для ВСЕХ страниц каждый sync (карта уже построена, это дёшево). Гаснет обновлением `timestamp` при обработке (правило замыкания класса A). detail = путь `s` с максимальным `last_touch` |
| `desc-mismatch` | A | (T5) проверяется для **всех** валидных страниц каждый sync, как `source-drift` (иначе правка `.desc.json` без правки страницы не сработает): у страницы есть `covers`, и (`last_touch` файла `<cover>/.desc.json` > `timestamp` страницы (epoch), ИЛИ `description` страницы изменился в этом прогоне). Гаснет только обновлением `timestamp` при обработке (правило замыкания класса A). detail = путь узла |
| `prose-changed` | B | страница изменена, `body_hash` изменился, `relations_hash` НЕ изменился, актор ≠ `maintenance` |
| `dup-candidate` | B | (T5) среди валидных изменённых страниц актора ≠ `maintenance`: существует другая страница с равным (без учёта регистра) `title` ИЛИ `description` — сравнение со **всем** множеством `pages`, не только с изменёнными в прогоне. detail = id второй страницы |
| `invalid-frontmatter` | B | §6.3 п. 4; снимается автоматически при успешной материализации (§6.3 п. 3) |
| `rename-candidate` | B | §6.3 п. 6 |
| `contradiction` | B | страница изменена (не `--full`), и среди её вставленных рёбер есть `contradicts` с `(type, dst)`, отсутствовавшим в прежнем множестве рёбер страницы (§6.3 п. 3). При `--full` не взводится (прежнее множество неизвестно; компенсация — проход пар `contradicts` в квартальном аудите уборщика). detail = `dst` ребра |

### 6.5 Завершение

1. `meta.last_processed_commit = HEAD`; счётчик `meta.changed_since_maintenance` (нужен `kb queue`, §8.5): при `--actor maintenance` сбросить в 0 (и `meta.last_maintenance_commit = HEAD`), иначе увеличить на число изменённых в прогоне страниц; при `--full` — `meta.last_full_rebuild = now()`.
2. `COMMIT`.
3. Прогнать lint (§7) и напечатать отчёт: `N страниц (M изменено, K удалено); ошибок: X; предупреждений: Y; в очереди: Z`.
4. Код выхода: 1, если есть ошибки валидатора у страниц, **изменённых в этом прогоне**; ошибки нетронутых страниц печатаются в отчёте, но код выхода не поднимают (чужая старая ошибка не блокирует несвязанную задачу; команда `kb lint` — полная диагностика, возвращает 1 при любых ошибках корпуса).
...
Предупреждения: **W1** — `target` ребра не существует в `pages`; **W2** — страница недостижима из `index.md` (BFS по markdown-ссылкам `](...)` из `index.md` и под-индексов — файлов `index.md` в поддиректориях бандла, §6.3 п. 1 — плюс по рёбрам); **W3** — ребро на страницу с `lifecycle: superseded`; **W4** — дубль `title`; **W5** — страница длиннее 200 строк; **W6** — `index.md` длиннее 150 строк; **W7** — доля рёбер `relates_to` > 30 % от всех; **W8** — ребро `contradicts`/`supersedes` без `provenance`; **W9** — страница без единого исходящего ребра; **W10** — зоны `covers:` двух страниц совпадают или пересекаются на одном уровне (вложенность разной глубины — НЕ предупреждение, действует deepest-match).
...
1. Построить FTS-запрос: токены = `re.findall(r'\w+', текст, re.UNICODE)`; каждый токен длиной ≥ 2 → `"токен"*` — **обязательно в двойных кавычках**: голый токен `OR`/`AND`/`NOT`/`NEAR` из текста запроса — синтаксическая ошибка FTS5 (крах вместо результата); токены длиной 1 — отбросить; соединить ` AND `. Пусто → сообщение «пустой запрос», код 0.
...
### 8.3 `kb page <id> [--section "<фрагмент заголовка>"]` — без `--section`: печать файла канона целиком; с `--section`: чанки страницы, где `heading` содержит фрагмент без учёта регистра; не найдено — список доступных `heading`, код 0. Путь `docs/knowledge/<id>.md` после `resolve()` обязан оставаться внутри бандла, иначе — `страница не найдена`, код 0 (защита от `../` в id); то же правило действует в §8.7 п. 1.

### 8.4 `kb log [<id>] [-n N]` — `git log -n <N> --format='%h %ad %s [%(trailers:key=KB-Actor,valueonly=true)]' --date=short -- docs/knowledge[/<id>.md]`; умолчание `N=20` (протокол читает команду каждую сессию — вывод обязан быть ограниченным).
...
- Проверку правила замыкания класса A (§7.5 проекта) `--done` сознательно **не** выполняет — обязанность обновить `timestamp` протокольная, блокирующую валидацию не добавлять: незакрытое условие безвредно пересчитается при следующем `--full`.

### 8.6 `kb eval [--k 3]` (T5)

Загрузить все `eval/cases/*.yaml` в порядке сортировки имён файлов. Файл, который не парсится или не содержит обоих полей `q`/`expect`, — пометить `[битый case: <имя файла>]` и исключить из знаменателя. Для каждого case: прогнать §8.1 п. 1–3 (без соседей), взять топ-k id. Попадание = пересечение с `expect` непусто. `expect`-id, отсутствующие в `pages`, — пометить `[нет такой страницы]` и исключить case из знаменателя. Вывод: `hit-rate: H/N = X.XX` + список промахов (`q`, `expect`, полученный топ); при N=0 — `hit-rate: 0/0 = n/a`. Код выхода всегда 0.

### 8.7 `kb impact <id | --draft <путь>> [--json]` (T4)

Проверочный набор перед записью (сверка на записи — §7.3 проекта). Определение цели:

1. Файл цели: режим `<id>` → `docs/knowledge/<id>.md` (файла нет → напечатать `страница не найдена: <id>`, код 0); режим `--draft <путь>` → указанный файл (файла нет → `файл не найден: <путь>`, код 2).
2. Прочитать файл (нормализация §5.1) и распарсить (§5.2). Невалиден → напечатать причину, код 1. Из результата взять `title`, `description`, `covers`, `relations`.
3. `id` цели: режим `<id>` — сам аргумент; режим `--draft` — путь относительно `docs/knowledge/` без `.md`, если файл внутри бандла, иначе пустая строка.

Три блока вывода (пустой блок печатается строкой `— нет`):

1. **Рёбра**: `SELECT src, type, dst FROM edges WHERE src = :id OR dst = :id ORDER BY type, src, dst` (пустой `id` → блок пуст); формат `src —type→ dst`. Дополнительно: `target` из `relations:` цели, отсутствующие в `pages`, перечислить строками `цель не существует: <target>`.
2. **Пересечения covers**: пройти страницы канона (glob `docs/knowledge/**/*.md`, исключая `schema/` и `index.md`; кроме самой цели), парсить §5.2 — невалидные молча пропускать; страница попадает в блок, если какой-то путь из её `covers` и какой-то путь из `covers` цели равны или один является сегментным префиксом другого (сравнение по сегментам `/` после снятия завершающего `/`; в отличие от W10 разноуровневая вложенность тоже показывается). Формат: `id — <путь цели> ~ <путь страницы>`. Проекция `covers` не хранит, блок читает канон.
3. **Лексические кандидаты**: FTS-запрос по §8.1 п. 1–3 от строки `title + " " + description` цели, соединитель сразу ` OR ` (AND-проход пропускается — токенов много), топ-5 страниц, исключая саму цель. Формат: `id — title [type]`.

`--json`: `{"edges":[{"src","type","dst"}],"missing_targets":["..."],"covers_overlap":[{"id","path"}],"lexical":[{"id","title","type"}]}`. Код выхода: 0 (невалидный черновик — 1; внутренняя ошибка — 2).

Стоп-слова из 2 символов в OR-запросе («на», «по») допустимы и специально не фильтруются: ранжирование bm25 взвешивает токены по редкости, частые слова не поднимают нерелевантные страницы, а короткие термины (`AI`, `БД`) обязаны участвовать в поиске дублей.
...
§6.2 (батч git), §6.4 (`human-edited`, `source-drift`, `prose-changed`, `invalid-frontmatter`, `contradiction`), §8.5, W6–W9.
**Проверки:** U15–U21, U34–U35; сценарный тест S1 (§10).

### T4 — читающие команды

§8.1–8.4, §8.7.
**Проверки:** U22–U26, U32–U33, U36.

### T5 — расширение (фаза 1.5)

Детекторы `dup-candidate`, `rename-candidate`, `desc-mismatch`; `kb eval`; W10.
**Проверки:** U27–U31; сценарный тест S2 (§10).

### T6 — интеграция и приёмка

e2e-тест E1 (§10); ADR `docs/adr/ADR-00XX-project-knowledge-base.md` (контекст, решение: SQLite-проекция + OKF-отступление по `log.md`; статус Proposed — принимает владелец); раздел «База проектных знаний» в AGENTS.md — скопировать приложение А проекта (v0.8), заменив внутренние ссылки вида «§7.5» ссылкой на проектный документ; обновить `.desc.json` затронутых директорий + `node scripts/dev/generate-structure.js`; финальный отчёт с фактическим выводом всех проверок.
...
- **U32** impact: цель с входящим ребром и страница с пересекающейся зоной `covers` (в т.ч. разной глубины) → блоки «Рёбра» и «Пересечения covers» непустые; **U33** impact `--draft`: новый файл, чей `title` разделяет слова с существующей страницей, → она в лексических кандидатах; несуществующий `target` в `relations:` черновика помечен `цель не существует`;
- **U34** contradiction: добавление нового ребра `contradicts` странице → `(src, 'contradiction', detail=dst)` в очереди; повторный sync без изменений второй раз не взводит; **U35** после починки битой страницы sync автоматически снимает её `invalid-frontmatter` из очереди; **U36** query с текстом, содержащим голое слово `OR`, не падает и возвращает результат (валидация кавычек §8.1 п. 1).
...
Сценарный **S2** (сверка на слиянии, §7.7 проекта): init git-репо → бандл → коммит с трейлером → sync → ветка `feature`: страница с ребром `contradicts` на существующую + страница с `title`, равным существующему, коммиты с трейлером → merge в main (`--no-ff`) → инкрементальный `sync --actor agent` на main → в очереди `contradiction` и `dup-candidate`. Контрольная ветвь теста: тот же merge в отдельной копии, но `sync --full` вместо инкрементального → `contradiction` НЕ взведён (подтверждение нормы «после merge — инкрементальный sync»).
...
9. U32/U33: `kb impact` выдаёт проверочный набор (рёбра, пересечения `covers`, лексические кандидаты).
10. U34: новое ребро `contradicts` доводит сигнал до очереди (`contradiction`) — критический путь протокола противоречия.
11. S2: сверка на слиянии работает — после merge ветки инкрементальный sync на main взводит `contradiction` и `dup-candidate` по слитым страницам; `--full` вместо инкрементального `contradiction` не взводит.
