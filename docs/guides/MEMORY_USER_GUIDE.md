...
## Прямое редактирование файлов знаний

Файл знания в `data/knowledge_repo/**/*.md` — канон (ADR-0030 решение 1): frontmatter (`scope`, владелец, `kind`, `sensitivity`, `scope_tokens`, `status`, `source_refs`, `supersedes`, `lifecycle`) и тело файла авторитетны, а база данных — пересобираемая проекция. Прямая правка файла владельцем канона (автором персонального знания или ответственным за организационный раздел) — штатный, а не аварийный путь:

- отредактировать `.md`-файл напрямую (текстовым редактором, скриптом или через внешний git-инструментарий) и закоммитить изменение в репозиторий знаний;
- выполнить `python manage.py memory_reconcile`, чтобы пересобрать проекцию (метаданные, полнотекстовый/векторный индекс, рёбра графа из `relations:`) под новым содержимым файла;
- расхождение хэша файла и старой проекции не является ошибкой чтения — reconciler подхватывает правку;
- единственное ограничение: ручная правка не может молча понизить `sensitivity`/`scope_tokens`. Такое понижение reconciler переводит в `lifecycle: pending` и держит на паузе (старая, более строгая классификация остается в силе) до явного решения в `/memory/review/pending/`; повышение классификации применяется автоматически.

Это делает файл знания редактируемым обычным текстовым инструментарием, а не только через AI-чат или `memory.remember`.
...
## Автоупорядочивание файлов (заморожено)

Контур вынесен из `apps.memory` в отдельное приложение `apps.filehub` (ADR-0030 решение 5) и заморожен: новые функции не разрабатываются до выбора реального пилотного источника и явного решения владельца. Функциональность ниже не изменилась — команды и UI работают как раньше, просто зарегистрированы в `apps.filehub`.
