...
Оба образа (`Dockerfile` для `web`, `services/agent_runtime/Dockerfile` для
`agent-runtime`) ставят зависимости из lock-файлов (`requirements.lock`,
`services/agent_runtime/requirements.lock`), а не из `requirements.txt`
напрямую — это фиксирует версии транзитивных зависимостей и делает сборку
воспроизводимой между хостами и запусками `docker build`. Подробности и
команда перегенерации (`make lock`, `make lock-agent-runtime`) — в README,
раздел «Воспроизводимая установка зависимостей».
...
## Env-файл хоста (`LOCAL_BUSINESS_ENV_FILE`)

`docker-compose.prod.yml` — общий для всех хостов файл и не должен содержать
путь к конкретному хосту (см. AGENTS.md, «Изоляция сред развертывания»:
host-specific `.env` живут только в `deployments/<host>/` и игнорируются
в `.gitignore`). Поэтому `env_file` сервисов `web` и `agent-runtime` указывает
...
Перед любым вызовом `docker compose -f docker-compose.prod.yml ...` нужно
задать переменную с путем к `.env` конкретного хоста:

```bash
export LOCAL_BUSINESS_ENV_FILE=deployments/<host>/.env
docker compose -f docker-compose.prod.yml config
docker compose -f docker-compose.prod.yml up -d --build
```

Без переменной `docker compose` завершится ошибкой с понятным текстом
(`required variable LOCAL_BUSINESS_ENV_FILE is missing a value: ...`), а не
тихо использует чужой или несуществующий env-файл.

[scripts/linux/deploy.sh](../../scripts/linux/deploy.sh) нацелен на текущий
единственный VPS и сам задаёт эту host-специфичную привязку: перед вызовами
`docker compose` в удалённой сессии он экспортирует
`export LOCAL_BUSINESS_ENV_FILE=deployments/test-host/.env`. Так путь к env-файлу
хоста остаётся в host-специфичном скрипте, а `docker-compose.prod.yml` — общий.
Для деплоя другого хоста скопируйте скрипт и поменяйте путь либо экспортируйте
переменную вручную в той же сессии перед запуском своей команды `docker compose`.
...
Все команды `docker compose -f docker-compose.prod.yml ...` требуют
`LOCAL_BUSINESS_ENV_FILE` в окружении (см. «Env-файл хоста» выше).

Логи:

```bash
ssh -i ~/.ssh/openclaw_vps_ed25519 -p 2222 admin@188.120.246.243 'cd /home/admin/local-business-suite && export LOCAL_BUSINESS_ENV_FILE=deployments/test-host/.env && sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml logs -f'
```

Статус контейнеров:

```bash
ssh -i ~/.ssh/openclaw_vps_ed25519 -p 2222 admin@188.120.246.243 'cd /home/admin/local-business-suite && export LOCAL_BUSINESS_ENV_FILE=deployments/test-host/.env && sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml ps'
```

Ручной rebuild:

```bash
ssh -i ~/.ssh/openclaw_vps_ed25519 -p 2222 admin@188.120.246.243 'cd /home/admin/local-business-suite && export LOCAL_BUSINESS_ENV_FILE=deployments/test-host/.env && sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml down --remove-orphans && sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml up -d --build'
```
...
sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml down --remove-orphans
sudo -E docker compose -p local-business-suite-prod -f docker-compose.prod.yml up -d --build
```
