...
class Command(BaseCommand):
    help = "Copy legacy analytics tables from main_vault.sqlite3 to data/db/analytics_control.sqlite3."
    ...
    def handle(self, *args, **options):
        ...
        target_db = settings.DATABASES["analytics_control"]["NAME"]
        report = copy_analytics_tables(legacy_db=legacy_db, target_db=target_db, dry_run=options["dry_run"])
        for table, counts in report.items():
            self.stdout.write(
                f"{table}: legacy={counts['legacy']}, target_before={counts['target_before']}, copied={counts['copied']}"
            )
        if not options["dry_run"]:
            self.stdout.write(self.style.SUCCESS("Legacy analytics control database migration finished."))


def copy_analytics_tables(*, legacy_db, target_db, dry_run):
    ...
    try:
        ...
        tables = [
            ...
            for row in connection.execute(
                "SELECT name FROM main.sqlite_master WHERE type = 'table' AND name LIKE 'analytics_%' ORDER BY name"
            )
        ]
        report = {}
        for table in tables:
            if not _table_exists(connection, "legacy", table):
                report[table] = {"legacy": 0, "target_before": _table_count(connection, "main", table), "copied": 0}
                continue
