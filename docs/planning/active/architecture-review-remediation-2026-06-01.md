...
### 2026-06-01. Частично выполнено

В первом кодовом срезе выполнены части пунктов 2, 3 и 4:

- raw prompt и полный actor context удалены из логов `services/agent_runtime/app.py`;
- ошибки runtime возвращают `request_id` и `conversation_id`, а не сырой текст исключения, и не пишут значение исключения в runtime/Django warning logs;
- AI tool `access.update_role_permissions` теперь вызывает Settings Center service layer и создает `SettingsChange` audit;
- Django AI gateway отклоняет отсутствующего, отключенного, некорректного или противоречивого actor до запуска инструмента;
- `AgentActionLog` для AI-команд хранит command metadata с confirmation state.

Остается выполнить: миграции всех runtime-баз и deployment-документацию, перенос debug PATH_INFO log из корня, обновление устаревших архитектурных ссылок и отдельное решение по внешнему MCP-доступу.
